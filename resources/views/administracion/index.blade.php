<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Bandeja Administración - Cheques') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form id="form-masivo" method="POST" action="{{ route('administracion.aprobarMasivo') }}">
                        @csrf
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <button type="submit" id="btn-masivo" class="hidden bg-primary-900 hover:bg-primary-950 text-white px-4 py-2 rounded-lg text-sm font-bold transition-all">
                                <i class="fas fa-paper-plane mr-2"></i> Enviar Seleccionados a Caja
                            </button>
                        </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="px-6 py-4 text-center text-sm">No hay cheques pendientes para Administración.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
    ...
    <script>
        function aprobarIndividual(id) {
            if(confirm('¿Aprobar y enviar cheque a Caja?')) {
                const form = document.getElementById('form-individual');
                form.action = `/administracion/${id}/aprobar`;
                form.submit();
            }
        }

        function rechazarCheque(id) {
            document.getElementById('form-rechazo').action = `/administracion/${id}/rechazar`;
