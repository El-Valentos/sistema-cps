<x-app-layout>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <span class="px-2 py-1 text-xs font-semibold rounded bg-sky-100 text-sky-800">
                    ...
                    <div class="mt-6 flex gap-3">
                        <form action="{{ route('administracion.aprobar', $cheque) }}" method="POST">
                            @csrf
                            <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700" onclick="return confirm('¿Aprobar y enviar a Caja?')">
                                Aprobar
                            </button>
                        </form>
                        ...
                        <a href="{{ route('administracion.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Volver</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="modal-rechazo" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            ...
            <form method="POST" action="{{ route('administracion.rechazar', $cheque) }}">
                @csrf
