<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Cheques - Revisión Finanziera') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form id="form-masivo" method="POST" action="{{ route('financiera.aprobarChequeMasivo') }}">
                        @csrf
                        <div class="flex justify-between items-center mb-4">
                            <p class="text-sm text-gray-600">Cheques aprobados por Presupuesto pendientes de revisión final.</p>
                            <div class="flex items-center gap-2">
                                ...
                                <button type="submit" id="btn-masivo" class="hidden bg-primary-900 hover:bg-primary-950 text-white px-4 py-2 rounded-lg text-sm font-bold transition-all ml-4">
                                    <i class="fas fa-paper-plane mr-2"></i> Enviar Seleccionados a Administración
                                </button>
                            </div>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="px-6 py-4 text-center text-sm">No hay cheques pendientes de revisión.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
    ...
    <script>
        function aprobarIndividual(id) {
            if(confirm('¿Aprobar y enviar cheque a Administración?')) {
                const form = document.getElementById('form-individual');
                form.action = `/financiera/cheques/${id}/aprobar`;
                form.submit();
            }
        }

        function rechazarCheque(id) {
            document.getElementById('form-rechazo').action = `/financiera/cheques/${id}/rechazar`;
